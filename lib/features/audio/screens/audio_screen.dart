...
void _showUnknownInstrumentDialog(PredictionResult result) {
  // Store the AudioCubit reference before showing the dialog
  final audioCubit = context.read<AudioCubit>();
  
  showDialog(
    context: context,
    barrierDismissible: false,
    builder: (dialogContext) => AlertDialog( // Changed parameter name for clarity
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(20),
      ),
      title: Row(
      ...
      actions: [
        ...
        ElevatedButton(
          onPressed: () {
            Navigator.of(dialogContext).pop(); // Use dialogContext for navigation
            audioCubit.reset(); // Use stored reference instead of context.read()
            _lastResultId = null; // Reset duplicate prevention
            _navigateToPage(0);
          },
          style: ElevatedButton.styleFrom(
